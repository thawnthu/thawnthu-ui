...
import { useRouter, useSearchParams } from "next/navigation"; // 1. belh

type FontSize = 'small' | 'medium' | 'large';

export default function SettingPage() {
  const router = useRouter(); // 2. belh
  const searchParams = useSearchParams(); // 3. belh
  const backTo = searchParams.get('back') || '/'; // 4. belh
  ...
  return (
    <div style={{background: bg, color: text, minHeight: '100vh'}}>
      ...
      <div style={{padding: '16px', display: 'flex', alignItems: 'center', gap: '10px', borderBottom: `1px solid ${border}`, background: bg, position: 'sticky', top: 0}}>
        <button onClick={()=>router.push(backTo)} style={{display: 'flex', alignItems: 'center', textDecoration: 'none', color: text, background: 'none', border: 'none', cursor: 'pointer'}}> {/* 5. Link aiin button */}
        ...
        <div style={{background: card, borderRadius: '12px', border: `1px solid ${border}`, overflow: 'hidden'}}>
          ...
          <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px'}}>
            <div style={{display: 'flex', alignItems: 'center', gap: '12px'}}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={text} strokeWidth="2">
              ...
              {(['small', 'medium', 'large'] as FontSize[]).map(size => (
                <button
                  ...
                  style={{
                    ...
                    padding: '10px',
                    borderRadius: '8px',
                    border: `1.5px solid ${fontSize === size? accent : border}`,
                    background: fontSize === size? accent : 'transparent',
                    color: fontSize === size? 'white' : text,
                    fontWeight: '600',
                    cursor: 'pointer',
                    fontSize: '14px',
