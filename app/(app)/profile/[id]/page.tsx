...
export default function FinalProfilePage() {
  ...
  useEffect(()=>{
    ...
    const unsub = onSnapshot(doc(db,"users",profileId), s=>{
      if(s.exists()){
        const d=s.data(); setUserData({id:s.id,...d});
  ...
  const save = async()=>{
    ...
    await updateDoc(doc(db,"users",currentUser.uid),{
      ...
      phone:form.phone.trim(), phonePublic:form.phonePublic, updatedAt:serverTimestamp()
    });
    ...
    <div style={{background:'#f0f2f5',minHeight:'calc(100vh - 135px)',paddingBottom:30}}>
      ...
      <div style={{marginTop:-55,padding:'0 12px'}}>
        <div style={{background:'#fff',borderRadius:22,padding:'16px 14px',boxShadow:'0 8px 24px rgba(0,0,0,0.08)',textAlign:'center'}}>
          ...
          {editAbout? <input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} style={{marginTop:10,width:'80%',textAlign:'center',fontSize:18,fontWeight:800,border:'1.5px solid #ddd',borderRadius:10,padding:'6px'}}/> : <h2 style={{margin:'10px 0 0',fontSize:19,fontWeight:800}}>{userData.name}</h2>}
      ...
      <div style={{padding:'10px 12px 0',display:'flex',flexDirection:'column',gap:10}}>
        ...
        <div style={{background:'#fff',borderRadius:16,padding:14}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
            ...
            {isOwn &&!editAbout && <button onClick={()=>setEditAbout(true)} style={{background:'#f3f0ff',border:'none',borderRadius:8,padding:'5px 10px',fontSize:11,fontWeight:700,color:'#8d31ce',display:'flex',alignItems:'center',gap:4}}><Edit3 size={12}/> Edit</button>}
          </div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                ...
                <div style={{gridColumn:'1 / span 2'}}>
                  <label style={{fontSize:11,color:'#888',display:'flex',justifyContent:'space-between'}}>Phone <span onClick={()=>setForm({...form,phonePublic:!form.phonePublic})} style={{color:form.phonePublic?'#22c55e':'#999',cursor:'pointer',display:'flex',gap:2,alignItems:'center'}}>{form.phonePublic? <><Eye size={10}/> Public</> : <><EyeOff size={10}/> Private</>}</span></label>
              ...
              <div style={{display:'flex',gap:8,marginTop:4}}>
                ...
                <button onClick={save} disabled={saving} style={{flex:1,background:'#8d31ce',color:'#fff',border:'none',borderRadius:10,padding:9,fontWeight:700,fontSize:13,display:'flex',justifyContent:'center',gap:5}}>{saving? <Loader2 size={14} className="animate-spin"/> : <><Save size={14}/> Save</>}</button>
              </div>
            </div>
          ):(
        ...
        {/* 2. FRIEND LIST - CLICK HNU AH CHIAH */}
        {showFriends && (
          <div style={{background:'#fff',borderRadius:16,padding:14}}>
        ...
        {/* Mi profile tan About chauh, Friends button hran */}
