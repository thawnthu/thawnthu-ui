...
export default function ProfilePage() {
  ...
  const onFile = async (e: any) => {
    ...
    reader.onload = (ev) => { const img = new Image(); img.onload = async () => {
        ...
        if (w > 500) { h = (h * 500) / w; w = 500; } c.width = w; c.height = h;
        c.getContext('2d')?.drawImage(img, 0, 0, w, h); const b64 = c.toDataURL('image/jpeg', 0.8);
  ...
  return (
    <div style={{ background: '#f0f2f5', minHeight: '100vh', paddingBottom: 20 }}>
      ...
      <div style={{ marginTop: -55, padding: '0 12px' }}>
        <div style={{ background: '#fff', borderRadius: 22, padding: 16, boxShadow: '0 8px 24px rgba(0,0,0,0.08)' }}>
              ...
              {friends.map((f: any, i: number) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: i === friends.length-1? 'none' : '1px solid #f3f4f6' }}>
                  <div onClick={() => router.push('/profile/' + f.uid)} style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, cursor: 'pointer' }}>
                    ...
                    <div><div style={{ fontSize: 15, fontWeight: 700 }}>{f.name}</div><div style={{ fontSize: 12, color: '#22c55e', fontWeight: 600 }}>Online</div></div>
                  </div>
          ...
          {isOwn && editAbout? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <input value={form.games} onChange={(e) => setForm({...form, games: e.target.value })} placeholder="Games" style={{ width: '100%', border: '1.5px solid #ddd', borderRadius: 10, padding: '10px 12px', fontSize: 14, boxSizing: 'border-box' }} />
              ...
              <div style={{ position: 'relative' }}>
                <input value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value })} placeholder="Phone" style={{ width: '100%', border: '1.5px solid #ddd', borderRadius: 10, padding: '10px 36px 10px 12px', fontSize: 14, boxSizing: 'border-box' }} />
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
                <button onClick={() => setEditAbout(false)} style={{ flex: 1, background: '#f3f4f6', border: 'none', borderRadius: 12, padding: 12, fontWeight: 700 }}>Cancel</button>
                <button onClick={save} style={{ flex: 1, background: '#8d31ce', color: '#fff', border: 'none', borderRadius: 12, padding: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>{saving? <Loader2 size={16} className="animate-spin" /> : <><Save size={16} /> Save</>}</button>
              </div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', gap: 12 }}><div style={{ width: 38, height: 38, background: '#f5f3ff', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FileText size={18} color="#8d31ce" /></div><div><div style={{ fontSize: 12, color: '#999', fontWeight: 600 }}>Bio</div><div style={{ fontSize: 15, fontWeight: 600, marginTop: 3 }}>{userData.bio||'Ka account thar'}</div></div></div>
              <div style={{ display: 'flex', gap: 12 }}><div style={{ width: 38, height: 38, background: '#fef3f2', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Cake size={18} color="#ef4444" /></div><div><div style={{ fontSize: 12, color: '#999', fontWeight: 600 }}>DOB</div><div style={{ fontSize: 15, fontWeight: 600, marginTop: 3 }}>{userData.dob||'Not set'}</div></div></div>
              <div style={{ display: 'flex', gap: 12 }}><div style={{ width: 38, height: 38, background: '#f0fdf4', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Home size={18} color="#22c55e" /></div><div><div style={{ fontSize: 12, color: '#999', fontWeight: 600 }}>Village</div><div style={{ fontSize: 15, fontWeight: 600, marginTop: 3 }}>{userData.village||'Not set'}</div></div></div>
              <div style={{ display: 'flex', gap: 12 }}><div style={{ width: 38, height: 38, background: '#eff6ff', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Gamepad2 size={18} color="#3b82f6" /></div><div><div style={{ fontSize: 12, color: '#999', fontWeight: 600 }}>Games</div><div style={{ fontSize: 15, fontWeight: 600, marginTop: 3 }}>{userData.favoriteGames||'Not set'}</div></div></div>
              <div style={{ display: 'flex', gap: 12 }}><div style={{ width: 38, height: 38, background: '#fdf2f8', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Heart size={18} color="#ec4899" /></div><div><div style={{ fontSize: 12, color: '#999', fontWeight: 600 }}>Hobby</div><div style={{ fontSize: 15, fontWeight: 600, marginTop: 3 }}>{userData.hobby||'Not set'}</div></div></div>
              {(isOwn || userData.phonePublic) && userData.phone && <div style={{ display: 'flex', gap: 12 }}><div style={{ width: 38, height: 38, background: '#f0fdf4', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Phone size={18} color="#22c55e" /></div><div><div style={{ fontSize: 12, color: '#999', fontWeight: 600 }}>Phone</div><div style={{ fontSize: 15, fontWeight: 600, marginTop: 3 }}>{userData.phone}</div></div></div>}
              <div style={{ display: 'flex', gap: 12 }}><div style={{ width: 38, height: 38, background: '#f3f0ff', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Mail size={18} color="#8d31ce" /></div><div><div style={{ fontSize: 12, color: '#999', fontWeight: 600 }}>Email</div><div style={{ fontSize: 15, fontWeight: 600, marginTop: 3 }}>{userData.email}</div></div></div>
            </div>
          )}
        </div>
      </div>
