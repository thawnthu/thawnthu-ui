...
import { confirmPasswordReset, verifyPasswordResetCode } from "firebase/auth"; // Pawimawh
...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    // 1. URL atang in oobCode kan la
    const code = searchParams.get('oobCode');
    if (code) {
      setOobCode(code);
      // Code hi a dik nge dik lo kan check lawk
  ...
  return (
    <div style={{background: bg, color: text, minHeight: '100vh', transition: '0.3s'}}>
      ...
      <div style={{
        ...
        <h1 style={{fontSize: '20px', fontWeight: '800', margin: 0}}>Reset Password</h1> {/* 1. HEADER THLAK */}
      </div>
      ...
      <div style={{display: 'flex', justifyContent: 'center', padding: '24px 20px'}}>
        <div style={{width: '100%', maxWidth: '380px'}}>
          <div style={{background: card, padding: '24px', borderRadius: '20px', border: `1px solid ${border}`, boxShadow: '0 8px 30px rgba(0,0,0,0.12)', transition: '0.3s'}}>
            ...
            <p style={{color: subtext, fontSize: '14px', marginBottom: '20px', marginTop: 0}}>Reset your password</p> {/* 1. SUBTEXT THLAK */}
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: '14px'}}>
              ...
              <button
                ...
                {loading? "Loading..." : "Reset Password"} {/* 1. BUTTON NAME THLAK */}
              </button>

              {success &&
                <p style={{color: '#22c55e', fontSize: '14px', marginTop: '4px', marginBottom: '0', fontWeight: '700', textAlign: 'center'}}>
                  Password reset successful! Please <Link href="/" style={{color: accent, fontWeight: '800', textDecoration: 'none'}}>Login</Link> {/* 3. LOGIN BOLD + CLICK THEIH */}
                </p>
              }
            </div>
          </div>
